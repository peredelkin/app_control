...
ALWAYS_INLINE static err_t modbus_rtu_message_recv(modbus_rtu_message_t* msg, modbus_io_t* io)
{
    return io->recv(io->usart, &msg->adu.func, MODBUS_RTU_PACKET_SIZE_MAX - 1);
}

ALWAYS_INLINE static void modbus_rtu_message_recv_done(modbus_rtu_message_t* msg, modbus_io_t* io)
{
    size_t bytes_received = io->bytes_received(io->usart) + 1;
...
{
    if(modbus_is->io == NULL) return E_NULL_POINTER;
...
err_t modbus_rtu_send_message(modbus_rtu_t* modbus)
{
    ...
    if(modbus->io->rx_busy(modbus->io->usart)) return E_BUSY;
    
    modbus->io->receiver_disable(modbus->io->usart);
    
    err_t err = modbus->io->send(modbus->io->usart, &modbus->tx_message->adu, modbus->tx_message->data_size + MODBUS_RTU_FIELDS_CRC_SIZE);
    
    if(err != E_NO_ERROR){
        modbus->io->receiver_enable(modbus->io->usart);
    }
...
bool modbus_rtu_usart_rx_byte_callback(modbus_rtu_t* modbus, uint8_t byte)
{
    if(modbus->mode == MODBUS_RTU_MODE_SLAVE &&
       modbus->address != byte && byte != MODBUS_RTU_ADDRESS_BROADCAST){
        modbus->io->sleep(modbus->io->usart);
        return true;
    }
    
    if(modbus->io->rx_errors(modbus->io->usart) != 0){
        modbus->io->sleep(modbus->io->usart);
        return true;
    }
    
    if(modbus_rtu_message_recv(modbus->rx_message, modbus->io) == E_NO_ERROR){
...
{
    if(modbus->io->rx_errors(modbus->io->usart) != 0) return true;
    
    modbus_rtu_message_recv_done(modbus->rx_message, modbus->io);
