...
//! IO шины.
typedef struct _Modbus_Io {
	void* usart;
	err_t (*send)(void* usart, const void* data, size_t size);
	err_t (*recv)(void* usart, void* data, size_t size);
	size_t (*bytes_received)(void* usart);
	bool (*rx_busy)(void* usart);
	void (*receiver_disable)(void* usart);
	void (*receiver_enable)(void* usart);
	void (*sleep)(void* usart);
	int (*tx_errors)(void* usart);
	int (*rx_errors)(void* usart);
} modbus_io_t;
 ...
 */
ALWAYS_INLINE static int modbus_rtu_rx_errors(modbus_rtu_t* modbus)
{
    return modbus->io->rx_errors(modbus->io->usart);
}
 ...
 */
ALWAYS_INLINE static int modbus_rtu_tx_errors(modbus_rtu_t* modbus)
{
    return modbus->io->tx_errors(modbus->io->usart);
}
