...
static bool tft9341_wait_current_op(tft9341_t* tft)
{
    ...
    if(spi_bus_is_busy(tft->spi) && spi_bus_transfer_id(tft->spi) != tft->transfer_id) return false;
 ...
 * @return Указатель на данные в бефере.
 */
ALWAYS_INLINE static SPI_BUS_FRAME_TypeDef* tft9341_get_message(tft9341_t* tft, size_t* index)
{
#ifdef TFT9341_GET_MEM_DEBUG
    if(index == NULL) return NULL;
#endif
...
err_t tft9341_nop(tft9341_t* tft)
{
    ...
    SPI_BUS_FRAME_TypeDef* cmd_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
...
err_t tft9341_read_id(tft9341_t* tft, tft9341_id_t* id)
{
    ...
    SPI_BUS_FRAME_TypeDef* data_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
...
err_t tft9341_read(tft9341_t* tft, void* data, size_t size)
{
    ...
    SPI_BUS_FRAME_TypeDef* trash_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
    if(data_msg == NULL) return E_OUT_OF_MEMORY;
#endif
...
err_t tft9341_set_pixel(tft9341_t* tft, uint16_t x, uint16_t y, const void* pixel, size_t size)
{
    ...
    SPI_BUS_FRAME_TypeDef* cmd_col_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
    ...
    SPI_BUS_FRAME_TypeDef* data_col_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
    ...
    SPI_BUS_FRAME_TypeDef* cmd_page_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
    ...
    SPI_BUS_FRAME_TypeDef* data_page_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
    ...
    SPI_BUS_FRAME_TypeDef* cmd_pixel_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
    ...
    SPI_BUS_FRAME_TypeDef* data_pixel_msg = tft9341_get_message(tft, &message_index);
#ifdef TFT9341_GET_MEM_DEBUG
