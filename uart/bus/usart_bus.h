...
typedef struct _UsartBus {
    USART_TypeDef* usart_device; //!< Устройство USART.
     void (*dma_rx_stream_callback);
    dma_t dma_rx_stream; //!< Канал DMA для приёма.
    dma_scr_chsel_t dma_rx_stream_channel; //!< Номер триггера DMA приема
    void (*dma_tx_stream_callback);
    dma_t dma_tx_stream; //!< Канал DMA для передачи.
    dma_scr_chsel_t dma_tx_stream_channel; //!< Номер триггера DMA передачи
