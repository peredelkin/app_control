                 ...
                 sdcard_reply_type_t reply_type, sdcard_reply_t* reply)
{
    ...
    sdio_respwait_t respwait = SDIO_RESP_WAIT_DIS;
    if(reply_type != SDCARD_REPLY_NO) respwait = SDIO_RESP_WAIT_ENA;

    sdio_resptype_t resptype = SDIO_RESP_TYPE_SHORT;
    if(reply_type == SDCARD_REPLY_R2) {
    	resptype = SDIO_RESP_TYPE_LONG;
    }

    sdio_command(cmd->argument, cmd->index, respwait, resptype, SDIO_INT_WAIT_DIS, SDIO_PEND_WAIT_DIS,
    				SDIO_CPSM_EN, SDIO_SUSPEND_DIS, SDIO_CMD_COMPLETION_DIS, SDIO_nIEN_DIS, SDIO_ATACMD_DIS);

    err = sdio_cmd_wait(reply_type);
    if(err != E_NO_ERROR) return err;
...
err_t sdcard_init_card(sdcard_t* sdcard)
{
    ...
    err = sdcard_cmd(sdcard, &sdcard_cmd0, SDCARD_REPLY_NO, NULL);
    if(err != E_NO_ERROR){
        sdcard_deselect(sdcard);
        sdcard_init_end_setup_spi(sdcard);
        return err;
